...
# note this only runs when knitting. you can't 'run' this chunk.
...
df_tee <- df_tee %>%
  mutate(conferral_eligibility = case_when(id %in% "mentee_9" ~ "Not eligible", # outcome of PhD submission?
...
eligibility criteria for the program. Below are the screening steps:
  ...
  *  Withdrawals: No one withdrew from the program before matching took place. 
  ...
  *  Mentee eligibility: We had
  ...
  `r level_no_n_tee` application from someone who holds a position above Level C, 
  `r phd_no_n_tee` application from people who have not yet completed their PhD, and
  `r contract_no_n_tee` application from people whose contracts will end before November 2025. 
...
knitr::kable (booktabs = T, linesep = "",
              ...
              align = 'lcc') %>% 
    add_header_above(c(" " = 1, "Mentors (n = 44)" = 2, "Mentees (n = 41)" = 2)) %>% 
  styler_pdf() 
